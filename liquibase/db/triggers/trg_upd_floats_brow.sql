CREATE OR REPLACE TRIGGER swms.trg_upd_floats_brow
------------------------------------------------------------------------------
-- Trigger Name:
--    trg_upd_floats_brow
--
...
--
-- Description:
--    Before update row trigger on the FLOATS table.
--    Assign:
--      - upd_user
--      - upd_date
--
   ...
   WHEN OTHERS THEN
      ...
      pl_log.ins_msg
               (i_msg_type         => pl_log.ct_warn_msg,
                i_procedure_name   => 'trg_upd_floats_brow',
                i_msg_text         => 'Error occurred in trigger.  NEW.float_no[' || TO_CHAR(:NEW.float_no) || ']'
                               || '  Processing will continue.',
                i_msg_no           => SQLCODE,
                i_sql_err_msg      => SQLERRM,
                i_application_func => 'ORDER PROCESSING',
                i_program_name     => 'trg_upd_floats_brow',
                i_msg_alert        => 'N');

END trg_upd_floats_brow;
/

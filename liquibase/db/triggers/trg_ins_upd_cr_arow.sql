CREATE OR REPLACE TRIGGER swms.trg_ins_upd_cr_arow
AFTER INSERT OR UPDATE ON swms.sap_cr_out
FOR EACH ROW
...
--                      Project: swms212- Staging table SAP_CR_OUT 
...
IF INSERTING THEN
    IF (SYS_CONTEXT('USERENV','MODULE')) NOT LIKE 'cashapcom%' THEN
        ...
        values('SAP_CR_OUT', replace(USER,'OPS$',NULL), 'I', :NEW.sequence_number, :OLD.batch_id,
...
IF UPDATING THEN
    IF (SYS_CONTEXT('USERENV','MODULE')) NOT LIKE 'cashapcom%' THEN
        ...
        staging_table, upd_user, Ins_upd_flag, sequence_number, old_batch_id, new_batch_id, 
        old_record_status, new_record_status, old_bypass_flag, new_bypass_flag, add_date,sys_context_parameter)
        values('SAP_CR_OUT', replace(USER,'OPS$',NULL), 'U', :NEW.sequence_number, :OLD.batch_id,
        :NEW.batch_id, :OLD.record_status, :NEW.record_status, NULL, NULL, SYSDATE,
        substr(SYS_CONTEXT('USERENV','MODULE'),1,50));
    END IF;
END IF;
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        ...
        pl_log.ins_msg('FATAL', 'trg_ins_upd_cr_arow', message, SQLCODE, SQLERRM, 'ORDER PROCESS', 'trg_ins_upd_cr_arow.sql', 'Y');
        
END trg_ins_upd_cr_arow;
/  

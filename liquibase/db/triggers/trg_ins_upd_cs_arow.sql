CREATE OR REPLACE TRIGGER swms.trg_ins_upd_cs_arow
AFTER INSERT OR UPDATE ON swms.SAP_CS_IN
FOR EACH ROW
...
--                      Project: swms212- Staging table SAP_CS_IN 
...
IF INSERTING THEN
    IF (SYS_CONTEXT('USERENV','MODULE')) NOT LIKE 'TRG_INS_SAP_CS_IN_BROW%' THEN
        ...
        values('SAP_CS_IN', replace(USER,'OPS$',NULL), 'I', :NEW.sequence_number, 
...
IF UPDATING THEN
    IF (SYS_CONTEXT('USERENV','MODULE')) NOT LIKE 'TRG_INS_SAP_CS_IN_BROW%' THEN
        ...
        staging_table, upd_user, Ins_upd_flag, sequence_number,old_record_status, 
        new_record_status, old_bypass_flag, new_bypass_flag, add_date,sys_context_parameter)
        values('SAP_CS_IN', replace(USER,'OPS$',NULL), 'U', :NEW.sequence_number,
        :OLD.record_status, :NEW.record_status, NULL, NULL, SYSDATE,
        substr(SYS_CONTEXT('USERENV','MODULE'),1,50));
    END IF;
END IF;
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        ...
        pl_log.ins_msg('FATAL', 'trg_ins_upd_cs_arow', message, SQLCODE, SQLERRM, 'MAINTENANCE', 'trg_ins_upd_cs_arow.sql', 'Y');
        
END trg_ins_upd_cs_arow;
/  

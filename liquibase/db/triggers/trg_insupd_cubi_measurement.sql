CREATE OR REPLACE TRIGGER swms.trg_insupd_cubi_measurement
AFTER INSERT OR UPDATE ON swms.CUBITRON_MEASUREMENT_IN
FOR EACH ROW
WHEN (USER != 'SWMS' AND USER != 'SWMS_JDBC' AND USER != 'OPS$SWMS')

------------------------------------------------------------------------------
-- Table:
--    CUBITRON_MEASUREMENT_IN
--
-- Description:
...
--                      Project: swms212- Staging table CUBITRON_MEASUREMENT_IN 
...
IF INSERTING THEN
    ...
    values('CUBITRON_MEASUREMENT_IN', replace(USER,'OPS$',NULL), 'I', :NEW.sequence_number, 
...
IF UPDATING THEN
    ...
    INSERT INTO SAP_TRACE_STAGING_TBL (
        staging_table, upd_user, Ins_upd_flag, sequence_number,old_record_status,
        new_record_status, old_bypass_flag, new_bypass_flag, add_date,sys_context_parameter)
    values('CUBITRON_MEASUREMENT_IN', replace(USER,'OPS$',NULL), 'U', :NEW.sequence_number, 
     :OLD.record_status, :NEW.record_status, NULL, NULL, SYSDATE,
     substr(SYS_CONTEXT('USERENV','MODULE'),1,50));
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        ...
        pl_log.ins_msg('FATAL', 'trg_insupd_cubi_measurement', message, SQLCODE, SQLERRM, 'CUBITRON', 'trg_insupd_cubi_measurement.sql', 'Y');
        
END trg_insupd_cubi_measurement;
/  

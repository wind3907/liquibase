/********************************************************************
**
** Trigger to allow update of records in gs1_finish_good_in table.
** Mainly to reprocess failed (F) records.
**
...
**
*********************************************************************/
CREATE OR REPLACE TRIGGER swms.trg_upd_gs1_finish_good_in
    BEFORE UPDATE ON swms.gs1_finish_good_in 
    FOR EACH ROW
...
BEGIN
    IF :OLD.record_status = 'S'  THEN
        :NEW.record_status := :OLD.record_status;
    ELSIF (:OLD.record_status = 'X' AND :NEW.record_status != 'N') THEN
        :NEW.record_status := :OLD.record_status;
    END IF;

    l_sequence_no := :OLD.sequence_number;
    ...
    IF :NEW.record_status = 'N' THEN
        ...
        IF l_count > 0 THEN
            l_error_msg := 'inv_cases record already exists for ' || 
                           ...
                           'box_id [' || :NEW.box_id || '] ' ||
                           'rec_id [' || :NEW.po_no || '] '; 
            pl_log.ins_msg('WARN', 'trg_upd_gs1_finish_good_in', l_error_msg, NULL, NULL);
...
EXCEPTION
   WHEN OTHERS THEN
       l_error_msg := 'Trigger trg_upd_gs1_finish_good_in FAILED. ' ||
                    'Sequence# = [' || :NEW.sequence_number || '] ' ||
                    'Prod_id = [' || :NEW.prod_id || '] ' ||
                    'Po_no = [' || :NEW.po_no || '] ' ||
                    'Order_id = [' || :NEW.order_id || '] ' ||
                    'Box_id = [' || :NEW.box_id || ']'; 
       pl_log.ins_msg('FATAL', 'trg_upd_gs1_finish_good_in', l_error_msg, SQLCODE, SQLERRM);

END trg_upd_gs1_finish_good_in;
/

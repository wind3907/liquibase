CREATE OR REPLACE TRIGGER swms.trg_ins_upd_ia_arow
AFTER INSERT OR UPDATE ON swms.sap_ia_out
FOR EACH ROW
...
--                      Project: swms212- Staging table SAP_IA_OUT 
...
IF INSERTING THEN
    IF (SYS_CONTEXT('USERENV','MODULE')) NOT LIKE 'swmstrwriter%' THEN
        ...
        values('SAP_IA_OUT', replace(USER,'OPS$',NULL), 'I', :NEW.sequence_number, :OLD.batch_id,
...
IF UPDATING THEN
    IF (SYS_CONTEXT('USERENV','MODULE')) NOT LIKE 'swmstrwriter%' THEN
        ...
        staging_table, upd_user, Ins_upd_flag, sequence_number, old_batch_id, new_batch_id, 
        old_record_status, new_record_status, old_bypass_flag, new_bypass_flag, add_date,sys_context_parameter)
        values('SAP_IA_OUT', replace(USER,'OPS$',NULL), 'U', :NEW.sequence_number, :OLD.batch_id,
        :NEW.batch_id, :OLD.record_status, :NEW.record_status, :OLD.bypass_flag, :NEW.bypass_flag, SYSDATE,
        substr(SYS_CONTEXT('USERENV','MODULE'),1,50));
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        ...
        pl_log.ins_msg('FATAL', 'trg_ins_upd_ia_arow', message, SQLCODE, SQLERRM, 'RECEIVING', 'trg_ins_upd_ia_arow.sql', 'Y');
        
END trg_ins_upd_ia_arow;
/  

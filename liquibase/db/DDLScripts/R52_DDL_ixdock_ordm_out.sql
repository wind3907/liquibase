/****************************************************************************
  File:
    R52_DDL_ixdock_ordm_out.sql

  Desc:
    Create the IXDOCK_ORDM_OUT table as a staging table to send ORDM data from
...
BEGIN
  ...
  INTO v_table_exists
  FROM all_tables
  WHERE table_name = 'IXDOCK_ORDM_OUT'
  AND owner = 'SWMS';

  IF (v_table_exists = 0) THEN
    -- Data from ORDM
    EXECUTE IMMEDIATE 'CREATE TABLE SWMS.IXDOCK_ORDM_OUT (
    ...
    EXECUTE IMMEDIATE 'COMMENT ON TABLE SWMS.IXDOCK_ORDM_OUT IS ''IXDOCK_ORDM_OUT staging table for cross dock site 2'' ';

    EXECUTE IMMEDIATE 'CREATE UNIQUE INDEX SWMS.IXDOCK_ORDM_OUT_PK ON SWMS.IXDOCK_ORDM_OUT
      (SEQUENCE_NUMBER)
      TABLESPACE SWMS_ITS2';

    EXECUTE IMMEDIATE 'CREATE INDEX SWMS.IXDOCK_ORDM_OUT_IDX1
      ON SWMS.IXDOCK_ORDM_OUT(batch_id)
      TABLESPACE SWMS_ITS2';

    EXECUTE IMMEDIATE 'CREATE INDEX SWMS.IXDOCK_ORDM_OUT_IDX2
      ON SWMS.IXDOCK_ORDM_OUT(record_status)
      TABLESPACE SWMS_ITS2';

    EXECUTE IMMEDIATE 'CREATE OR REPLACE PUBLIC SYNONYM IXDOCK_ORDM_OUT FOR SWMS.IXDOCK_ORDM_OUT';

    EXECUTE IMMEDIATE 'ALTER TABLE SWMS.IXDOCK_ORDM_OUT ADD (
        CONSTRAINT IXDOCK_ORDM_OUT_PK
        PRIMARY KEY (SEQUENCE_NUMBER)
        USING INDEX TABLESPACE SWMS_ITS2
      )';

    EXECUTE IMMEDIATE  'GRANT DELETE, INSERT, SELECT, UPDATE ON SWMS.IXDOCK_ORDM_OUT TO SWMS_USER';

    EXECUTE IMMEDIATE 'GRANT SELECT ON SWMS.IXDOCK_ORDM_OUT TO SWMS_VIEWER';
  END IF;
END;
/

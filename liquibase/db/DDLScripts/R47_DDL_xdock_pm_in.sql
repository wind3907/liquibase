...
BEGIN
  ...
  SELECT COUNT(*)
   ...
   WHERE object_type = 'TABLE'
     AND owner       = 'SWMS'
     AND object_name = 'XDOCK_PM_IN';
     
  IF N_COUNT > 0 THEN
    Dbms_Output.Put_Line( 'Table SWMS.XDOCK_PM_IN found, skipping recreation...' );
  ELSE
    Dbms_Output.Put_Line( 'Table SWMS.XDOCK_PM_IN not found, creating now...' );
       EXECUTE IMMEDIATE  'CREATE TABLE XDOCK_PM_IN 
...
EXECUTE IMMEDIATE 'COMMENT ON TABLE SWMS.XDOCK_PM_IN IS ''XDOCK_PM_IN Input staging table for cross dock site 2'' ';

EXECUTE IMMEDIATE 'CREATE UNIQUE INDEX SWMS.XDOCK_PM_IN_PK ON SWMS.XDOCK_PM_IN
                   (SEQUENCE_NUMBER)
                   LOGGING
                   TABLESPACE SWMS_ITS2';

EXECUTE IMMEDIATE 'CREATE OR REPLACE PUBLIC SYNONYM XDOCK_PM_IN FOR SWMS.XDOCK_PM_IN';

EXECUTE IMMEDIATE ' ALTER TABLE SWMS.XDOCK_PM_IN ADD (
  CONSTRAINT XDOCK_PM_IN_PK
  PRIMARY KEY
  (SEQUENCE_NUMBER)
  USING INDEX SWMS.XDOCK_PM_IN_PK
  ENABLE VALIDATE)';

EXECUTE IMMEDIATE 'CREATE INDEX SWMS.XDOCK_PM_IN_IDX1 ON SWMS.XDOCK_PM_IN
					(BATCH_ID)
					LOGGING
					TABLESPACE SWMS_ITS2';

EXECUTE IMMEDIATE 'GRANT DELETE, INSERT, SELECT, UPDATE ON SWMS.XDOCK_PM_IN TO SWMS_USER';

EXECUTE IMMEDIATE 'GRANT SELECT ON SWMS.XDOCK_PM_IN TO SWMS_VIEWER';

/****************************************************************************
  File:
    R47_DDL_xdock_ordd_in.sql.sql

  Desc:
    Create the XDOCK_ORDD_IN table as a staging table to receive ORDD data from
    site 1(Fulfillment) xdock_ordd_out to site 2(Last mile) xdock_ordd_in.
...
BEGIN
  ...
  INTO v_table_exists
  FROM all_tables
  WHERE table_name = 'XDOCK_ORDD_IN'
  AND owner = 'SWMS';

  IF (v_table_exists = 0) THEN
    EXECUTE IMMEDIATE 'CREATE TABLE SWMS.XDOCK_ORDD_IN (
    ...
    EXECUTE IMMEDIATE 'COMMENT ON TABLE SWMS.XDOCK_ORDD_IN IS ''XDOCK_ORDD_IN staging table to receive ORDD information from site 1 to site 2'' ';

    EXECUTE IMMEDIATE 'CREATE UNIQUE INDEX SWMS.XDOCK_ORDD_IN_PK
      ON SWMS.XDOCK_ORDD_IN (SEQUENCE_NUMBER)
      TABLESPACE SWMS_ITS2';

    EXECUTE IMMEDIATE 'CREATE INDEX SWMS.XDOCK_ORDD_IN_IDX1
      ON SWMS.XDOCK_ORDD_IN(batch_id)
      TABLESPACE SWMS_ITS2';

    EXECUTE IMMEDIATE 'CREATE INDEX SWMS.XDOCK_ORDD_IN_IDX2
      ON SWMS.XDOCK_ORDD_IN(record_status)
      TABLESPACE SWMS_ITS2';

    EXECUTE IMMEDIATE 'CREATE OR REPLACE PUBLIC SYNONYM XDOCK_ORDD_IN FOR SWMS.XDOCK_ORDD_IN';

    EXECUTE IMMEDIATE 'ALTER TABLE SWMS.XDOCK_ORDD_IN ADD (
      CONSTRAINT XDOCK_ORDD_IN_PK
      PRIMARY KEY (SEQUENCE_NUMBER)
      USING INDEX TABLESPACE SWMS_ITS2
    )';

    EXECUTE IMMEDIATE  'GRANT DELETE, INSERT, SELECT, UPDATE ON SWMS.XDOCK_ORDD_IN TO SWMS_USER';

    EXECUTE IMMEDIATE 'GRANT SELECT ON SWMS.XDOCK_ORDD_IN TO SWMS_VIEWER';
  END IF;
END;
/

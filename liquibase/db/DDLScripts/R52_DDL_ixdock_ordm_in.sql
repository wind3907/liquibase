/****************************************************************************
  File:
    R52_DDL_ixdock_ordm_in.sql

  Desc:
    Create the IXDOCK_ORDM_IN table as a staging table to recieve ORDM data from
    site 1(Fulfillment) ixdock_ordm_out to site 2(Last mile) ixdock_ordm_in.
...
BEGIN
  ...
  INTO v_table_exists
  FROM all_tables
  WHERE table_name = 'IXDOCK_ORDM_IN'
  AND owner = 'SWMS';

  IF (v_table_exists = 0) THEN
    EXECUTE IMMEDIATE 'CREATE TABLE SWMS.IXDOCK_ORDM_IN (
    ...
    EXECUTE IMMEDIATE 'COMMENT ON TABLE SWMS.IXDOCK_ORDM_IN IS ''IXDOCK_ORDM_IN staging table to receive ORDM table information from site 1 to site 2'' ';

    EXECUTE IMMEDIATE 'CREATE UNIQUE INDEX SWMS.IXDOCK_ORDM_IN_PK
      ON SWMS.IXDOCK_ORDM_IN(SEQUENCE_NUMBER)
      TABLESPACE SWMS_ITS2';

    EXECUTE IMMEDIATE 'CREATE INDEX SWMS.IXDOCK_ORDM_IN_IDX1
      ON SWMS.IXDOCK_ORDM_IN(batch_id)
      TABLESPACE SWMS_ITS2';

    EXECUTE IMMEDIATE 'CREATE INDEX SWMS.IXDOCK_ORDM_IN_IDX2
      ON SWMS.IXDOCK_ORDM_IN(record_status)
      TABLESPACE SWMS_ITS2';

    EXECUTE IMMEDIATE 'CREATE OR REPLACE PUBLIC SYNONYM IXDOCK_ORDM_IN FOR SWMS.IXDOCK_ORDM_IN';

    EXECUTE IMMEDIATE 'ALTER TABLE SWMS.IXDOCK_ORDM_IN ADD (
      CONSTRAINT IXDOCK_ORDM_IN_PK
      PRIMARY KEY (SEQUENCE_NUMBER)
      USING INDEX TABLESPACE SWMS_ITS2
    )';

    EXECUTE IMMEDIATE  'GRANT DELETE, INSERT, SELECT, UPDATE ON SWMS.IXDOCK_ORDM_IN TO SWMS_USER';

    EXECUTE IMMEDIATE 'GRANT SELECT ON SWMS.IXDOCK_ORDM_IN TO SWMS_VIEWER';
  END IF;
END;
/

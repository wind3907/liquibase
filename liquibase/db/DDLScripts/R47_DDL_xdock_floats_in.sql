/****************************************************************************
  ...
  File:
    R47_DDL_xdock_floats_in.sql

  Desc:
    Create the XDOCK_FLOATS_IN table as a staging table to send FLOAT data
    from site 1(Fulfillment) xdock_floats_out to site 2(Last mile) xdock_floats_in.
...
BEGIN
  ...
  INTO v_table_exists
  FROM all_tables
  WHERE table_name = 'XDOCK_FLOATS_IN'
  AND owner = 'SWMS';

  IF (v_table_exists = 0) THEN
    EXECUTE IMMEDIATE 'CREATE TABLE SWMS.XDOCK_FLOATS_IN (
    ...
    EXECUTE IMMEDIATE 'COMMENT ON TABLE SWMS.XDOCK_FLOATS_IN IS ''XDOCK_FLOATS_IN staging table to receive float table information from site 1 to site 2'' ';

    EXECUTE IMMEDIATE 'CREATE UNIQUE INDEX SWMS.XDOCK_FLOATS_IN_PK
      ON SWMS.XDOCK_FLOATS_IN (SEQUENCE_NUMBER)
      TABLESPACE SWMS_ITS2';

    EXECUTE IMMEDIATE 'CREATE INDEX SWMS.XDOCK_FLOATS_IN_IDX1
      ON SWMS.XDOCK_FLOATS_IN(batch_id)
      TABLESPACE SWMS_ITS2';

    EXECUTE IMMEDIATE 'CREATE INDEX SWMS.XDOCK_FLOATS_IN_IDX2
      ON SWMS.XDOCK_FLOATS_IN(record_status)
      TABLESPACE SWMS_ITS2';

    EXECUTE IMMEDIATE 'CREATE OR REPLACE PUBLIC SYNONYM XDOCK_FLOATS_IN FOR SWMS.XDOCK_FLOATS_IN';

    EXECUTE IMMEDIATE 'ALTER TABLE SWMS.XDOCK_FLOATS_IN ADD (
      CONSTRAINT XDOCK_FLOATS_IN_PK
      PRIMARY KEY (SEQUENCE_NUMBER)
      USING INDEX TABLESPACE SWMS_ITS2
    )';

    EXECUTE IMMEDIATE  'GRANT DELETE, INSERT, SELECT, UPDATE ON SWMS.XDOCK_FLOATS_IN TO SWMS_USER';

    EXECUTE IMMEDIATE 'GRANT SELECT ON SWMS.XDOCK_FLOATS_IN TO SWMS_VIEWER';
  END IF;
END;
/

/****************************************************************************
  ...
  File:
    xdock_manifest_dtls_out.sql

  Desc:
    Create the XDOCK_MANIFEST_DTLS_OUT table as a staging table to send Manifest Details data
...
BEGIN
    ...
    INTO v_table_exists
    FROM all_tables
    WHERE table_name = 'XDOCK_MANIFEST_DTLS_OUT'
      AND owner = 'SWMS';

    IF(v_table_exists = 0) THEN
        EXECUTE IMMEDIATE '
        CREATE TABLE SWMS.XDOCK_MANIFEST_DTLS_OUT
        (
        ...
        EXECUTE IMMEDIATE 'ALTER TABLE SWMS.XDOCK_MANIFEST_DTLS_OUT
            ADD CONSTRAINT XDOCK_MANIFEST_DTLS_OUT_PK PRIMARY KEY (batch_id,sequence_no)';

        EXECUTE IMMEDIATE 'CREATE INDEX SWMS.XDOCK_MANIFEST_DTLS_OUT_IDX2
            ON SWMS.XDOCK_MANIFEST_DTLS_OUT (record_status)';

        EXECUTE IMMEDIATE 'CREATE
        OR REPLACE PUBLIC SYNONYM XDOCK_MANIFEST_DTLS_OUT FOR SWMS.XDOCK_MANIFEST_DTLS_OUT';

        EXECUTE IMMEDIATE 'GRANT DELETE, INSERT, SELECT, UPDATE ON SWMS.XDOCK_MANIFEST_DTLS_OUT TO SWMS_USER';

        EXECUTE IMMEDIATE 'GRANT SELECT ON SWMS.XDOCK_MANIFEST_DTLS_OUT TO SWMS_VIEWER';
    END IF;
END;
/

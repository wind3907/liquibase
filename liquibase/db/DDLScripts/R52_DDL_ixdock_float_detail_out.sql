/****************************************************************************
  ...
  File:
    R52_DDL_ixdock_float_detail_out.sql

  Desc:
    Create the IXDOCK_FLOAT_DETAIL_OUT table as a staging table to send FLOAT data
...
BEGIN
  ...
  INTO v_table_exists
  FROM all_tables
  WHERE table_name = 'IXDOCK_FLOAT_DETAIL_OUT'
  AND owner = 'SWMS';

  IF (v_table_exists = 0) THEN
    -- Data from FLOAT table
    EXECUTE IMMEDIATE 'CREATE TABLE SWMS.IXDOCK_FLOAT_DETAIL_OUT (
    ...
    EXECUTE IMMEDIATE 'COMMENT ON TABLE SWMS.IXDOCK_FLOAT_DETAIL_OUT IS ''IXDOCK_FLOAT_DETAIL_OUT staging table to send float table information from site 1 to site 2'' ';

    EXECUTE IMMEDIATE 'CREATE UNIQUE INDEX SWMS.IXDOCK_FLOAT_DETAIL_OUT_PK ON SWMS.IXDOCK_FLOAT_DETAIL_OUT
      (SEQUENCE_NUMBER)
      TABLESPACE SWMS_ITS2';

    EXECUTE IMMEDIATE 'CREATE INDEX SWMS.IXDOCK_FLOAT_DETAIL_OUT_IDX1
      ON SWMS.IXDOCK_FLOAT_DETAIL_OUT(batch_id)
      TABLESPACE SWMS_ITS2';

    EXECUTE IMMEDIATE 'CREATE INDEX SWMS.IXDOCK_FLOAT_DETAIL_OUT_IDX2
      ON SWMS.IXDOCK_FLOAT_DETAIL_OUT(record_status)
      TABLESPACE SWMS_ITS2';

    EXECUTE IMMEDIATE 'CREATE OR REPLACE PUBLIC SYNONYM IXDOCK_FLOAT_DETAIL_OUT FOR SWMS.IXDOCK_FLOAT_DETAIL_OUT';

    EXECUTE IMMEDIATE 'ALTER TABLE SWMS.IXDOCK_FLOAT_DETAIL_OUT ADD (
        CONSTRAINT IXDOCK_FLOAT_DETAIL_OUT_PK
        PRIMARY KEY (SEQUENCE_NUMBER)
        USING INDEX TABLESPACE SWMS_ITS2
      )';

    EXECUTE IMMEDIATE  'GRANT DELETE, INSERT, SELECT, UPDATE ON SWMS.IXDOCK_FLOAT_DETAIL_OUT TO SWMS_USER';

    EXECUTE IMMEDIATE 'GRANT SELECT ON SWMS.IXDOCK_FLOAT_DETAIL_OUT TO SWMS_VIEWER';
  END IF;
END;
/

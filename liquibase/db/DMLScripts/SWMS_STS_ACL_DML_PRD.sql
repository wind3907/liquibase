...
Begin
  ...
  If acl_count = 0 then
    ...
    $if swms.platform.SWMS_REMOTE_DB $then
      BEGIN
        dbms_network_acl_admin.assign_acl(acl => 'swms_sts_server_a.xml', host => 'ms247stsswms04.na.sysco.net', lower_port => 8000,upper_port => 8000);
      END;
    $else
      BEGIN
        dbms_network_acl_admin.assign_acl(acl => 'swms_sts_server_a.xml', host => 'ms247stsswms04', lower_port => 8000,upper_port => 8000);
      END;
    $end
    commit; 
  End If;
  ...
  If acl_count2 = 0 then
    ...
    $if swms.platform.SWMS_REMOTE_DB $then
      BEGIN
        dbms_network_acl_admin.assign_acl(acl => 'swms_sts_server_b.xml', host => 'ms247stsswms02.na.sysco.net', lower_port => 8000,upper_port => 8000);
      END;
    $else
      BEGIN
        dbms_network_acl_admin.assign_acl(acl => 'swms_sts_server_b.xml', host => 'ms247stsswms02', lower_port => 8000,upper_port => 8000);
      END;
    $end
    commit; 
  End If;

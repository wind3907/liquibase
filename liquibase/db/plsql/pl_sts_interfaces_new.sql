CREATE OR REPLACE PACKAGE      swms.pl_sts_interfaces_new
AS
  ...
  --									      and send RTN and STC(stop close) at real time
   ...
   --   05/21/20   spin4795 Jira-2872 Changes made to prevent the creation of irrelevant AL (Alternate Location)
   --                       records in the STS_ROUTE_OUT staging table.  A proper AL record should have either 
   --                       a float ID or a zone_name to accompany the barcode.
   --   05/21/20   spin4795 Jira-2872 Changes made to always create a Delivery Pallet DI record for every float
   --                       that is accessed at each stop.
   -----------------------------------------------------------------------------
...
END pl_sts_interfaces_new;
/


CREATE OR REPLACE PACKAGE BODY      swms.pl_sts_interfaces_new
AS
---------------------------------------------------------------------------
-- Private Modules
-------------------------------------------------------------------------------
...
/* Charm6000008485- added the below variable */
   ...
   l_ship_name customers.ship_name%TYPE;
   l_ship_addr1 customers.ship_addr1%TYPE;
   l_ship_addr2 customers.ship_addr2%TYPE;
   l_stop_csz VARCHAR2(30);
   l_cust_contact customers.cust_contact%TYPE;

   l_item_id sts_route_out.item_id%TYPE;
   l_itemdesc sts_route_out.descrip%TYPE;
   l_zone_name sts_route_out.zone_name%TYPE;
   l_allowunload sts_route_out.allow_unload%TYPE;
   l_splitflag sts_route_out.splitflag%TYPE;
   l_compartment sts_route_out.compartment%TYPE;
   ...
   IS
      SELECT  distinct  stop_no, cust_id,
--      regexp_replace(ship_name,'[[:space:]]+',' ') ship_name,
...
--                      cust_contact,
        manifest_no,to_char(route_date,'mm/dd/yyyy') route_date
             FROM sts_route_view
   ...
   IS
      SELECT DISTINCT stop_no, float_seq
                 FROM sts_route_view
                WHERE route_no = i_route_no
                  AND route_date = i_route_date
                  AND stop_no = i_stop_no
                  AND sts_sort = '1'
                  order by stop_no, float_seq;
   ...
   IS
   SELECT DISTINCT s.float_no, s.float_seq, rpad(s.truck_no,4,' ') || s.float_seq barcode,
                      'F' || rpad(s.truck_no,4,' ') || s.float_seq item_id, '0' prod_id,
                      'N' bulk_item, 'N' order_line_state
                 FROM floats f, sts_route_view s
                WHERE f.float_no = s.float_no
                  AND s.route_no = i_route_no
                  AND s.route_date = i_route_date
                  AND f.float_seq IS NOT NULL
                  AND s.sts_sort NOT IN ('2', '3')
 ...
 BEGIN
  ...
  --Jira 2872  the ship date( i_route_Shpdt) derived is passed in every cursor below as route_Date
   ...
   LOOP
      ...
      LOOP
           BEGIN
                    SELECT regexp_replace(ship_name,'[[:space:]]+',' '),
                           regexp_replace(ship_addr1,'[[:space:]]+',' '),
                           regexp_replace(ship_addr2,'[[:space:]]+',' '),
                           substr(TRIM (ship_city ||' '|| ship_state ||' '|| ship_zip),1,30),
                           cust_contact
                      INTO l_ship_name, l_ship_addr1, l_ship_addr2, l_stop_csz, l_cust_contact
                      FROM customers
                     WHERE cust_id = st.cust_id;
           EXCEPTION
             WHEN OTHERS THEN
               l_ship_name := NULL;
               l_ship_addr1 := NULL;
				...
				--CRQ22702 removed the cust id validation for building the stop record.
			      ...
			      INSERT INTO sts_route_out
			          ...
			          VALUES (sts_route_out_seq.NEXTVAL, 'STS', 'N', SYSDATE,r.route_no,r.route_date,
			                  'ST', l_curr_stop_no, st.cust_id, l_ship_name,
			                  l_ship_addr1, l_ship_addr2, l_stop_csz,
			                  l_cust_contact, st.stop_no, st.manifest_no,
...
--                 IF dp.float_no <> 0 then   -- to handle rare cases where sometimes float seq is not null but of 4 blank spaces - opco 045
                  ...
                  BEGIN
                    SELECT 'F' || rpad(s.truck_no,4,' ') || s.float_seq,
                           'Pallet ' || s.truck_no || ' ' || s.float_seq,
                           s.truck_zone,
                           DECODE (s.unitize_ind,
                                   'Y', 'Y',
                                   DECODE (s.pallet_pull, 'B', 'Y', 'N')
                                  ),
                           DECODE (s.pallet_pull, 'B', 'N', 'Y'),
                           SUBSTR (s.float_seq, 1, 1)
                      INTO l_item_id, l_itemdesc, l_zone_name, l_allowunload,
                           l_splitflag, l_compartment
                      FROM STS_ITEMS S
                     WHERE route_no = i_route_no
                       AND route_date = i_route_Shpdt
                       AND stop_no = st.stop_no
                       AND float_seq = dp.float_seq
                       AND ROWNUM = 1;
                  EXCEPTION
                    WHEN OTHERS THEN
                      l_item_id := NULL;
                  ...
                  INSERT INTO sts_route_out
                       ...
                       VALUES (sts_route_out_seq.NEXTVAL, 'STS', 'N', r.route_no, r.route_date,
                               SYSDATE, 'DI', l_item_id,
                               ' ', 'N',
                               'N', 'C', l_itemdesc,
                               l_zone_name, l_allowunload,
                               'N', l_splitflag,
                               NULL, NULL,
                               ' ', l_compartment, 1, 1, ' ', '0',
                               'N', REPLACE (USER, 'OPS$', NULL),
                               SYSDATE, REPLACE (USER, 'OPS$', NULL), SYSDATE
                              );
 
                  MESSAGE := 'INSERTING DELIVERY PALLET RECORD '
                        || r.route_no
                        || ' Route Date: '
                        || r.route_date
                        || ' Item: '
...
--                END IF;
           ...
           END LOOP;                          --delivery pallet loop close
                 ...
                 MESSAGE := 'INSERTED DELIVERY ITEM RECORDS FOR Route: '
                    || r.route_no
                    || ' Route Date:'
                    || r.route_date;
           ...
           ELSE
               ...
               FOR i in 1..cc.qty
                loop
                   ...
                   --Charm#6000008485 - Otherthan High quantity selecting the individual Catchweightand catch_wt_trk filed -START
                   ...
                   --Charm#6000008485 - Added catchweight column and catch_wt_trk field  in the below insert query
                    ...
                    INSERT INTO sts_route_out
                       ...
                       VALUES (sts_route_out_seq.NEXTVAL, 'STS', 'N',cc.float_id,
                               SYSDATE, 'BC', bi.ordd_seq || LPAD (l_seq, 3, 0),
                               bi.ordd_seq || LPAD (cc.l_bc_seq, 4, 0),
                               bi.prod_id, bi.qty, bp.bulk_item,
                               bp.order_line_state,v_catch_wt_trk,v_catch_weight,
                               REPLACE (USER, 'OPS$', NULL), SYSDATE,
                               REPLACE (USER, 'OPS$', NULL), SYSDATE,r.route_no,r.route_date
...
CREATE OR REPLACE PUBLIC SYNONYM pl_sts_interfaces_new FOR swms.pl_sts_interfaces_new;
GRANT EXECUTE ON swms.pl_sts_interfaces_new TO SWMS_VIEWER;
GRANT EXECUTE ON swms.pl_sts_interfaces_new TO SWMS_USER;

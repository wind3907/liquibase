...
--
-- Description:
--    This script creates procedure "p_split" that moves the records from
--    the SWMS HOI staging table to the miniloader HOI staging table.
-- 
--    "p_split" is called by shell script "/swms/curr/bin/ml_int.sh" which is run by cron.
--
--    There are three different versions of p_split that can be created based on
...
--    appropriate version of p_split is created.
-- 
...
--    -------- -------- ---------------------------------------------------
--    10/26/07          Created as part of the HK Automation
--
...
--                      
--                      What we will do is create the appropriate verson of "p_split"
--                      based on:
--                      - Miniloader using ORACLE
--                      - Miniloader using SQLQERVER and not OpCo 024.
--                      - Miniloader using SQLSERVER and it is OpCo 024.
--
--                      What we had been doing is having different "pl_ml_split.sql"
--                      for ORACLE, SQLSERVER and OpCo 024 which caused issues
--                      after a SWMS upgrade as we had to be sure to run the correct
--                      "pl_ml_split.sql" after the upgrade.  This was a manual process.
...
DECLARE
   l_file_name       VARCHAR2(20) := 'pl_ml_split.sql';

   l_ml_database     VARCHAR2(20);
   l_sqlstmt_to_run  VARCHAR2(25000);

   ------------------------------------------------------------------------
   -- SQL stmt to create procedure p_split when miniloader using ORACLE
   ------------------------------------------------------------------------
   l_sqlstmt_oracle  VARCHAR2(25000) :=
'CREATE OR REPLACE PROCEDURE swms.p_split
IS
   CURSOR c_ml_config
   IS
...
BEGIN
   DBMS_OUTPUT.PUT_LINE(''Executing p_split, miniloader using ORACLE...'');

   FOR c1 IN c_ml_config
   LOOP
      DBMS_OUTPUT.PUT_LINE('' *****  Writing to Miniload System '' || c1.ml_system || ''  ***** '');

      sqlStmt := ''insert into '' || c1.swms_ml_synonym || ''( hoi_id, creation_timestamp, length, data ) (select hoi_id, creation_timestamp, length, data from HOI where ml_system ='' || '''''''' || c1.ml_system || '''''''' || '')'';				

      EXECUTE IMMEDIATE sqlStmt;

      DBMS_OUTPUT.PUT_LINE('' *****  Deleting data from SWMS  ***** '');

      sqlStmt := ''delete from HOI where ml_system ='' || '''''''' || c1.ml_system || '''''''' ||'' and 
      HOI_ID<=(select max(hoi_id) from '' || c1.swms_ml_synonym || '')'';
...
/*EXCEPTION
WHEN OTHERS THEN
DBMS_OUTPUT.PUT_LINE (''Unable to Write to Miniload DB'');*/
		
END p_split;';


   -----------------------------------------------------------------------------
   -- SQL stmt to create procedure p_split when miniloader using SQLSERVER
   -- and it is not OpCo 024
   -----------------------------------------------------------------------------
   l_sqlstmt_sqlserver  VARCHAR2(25000) :=
'CREATE OR REPLACE PROCEDURE swms.p_split
IS
   l_first_hoi_id                NUMBER;
   l_last_hoi_id                 NUMBER;
   l_number_of_records_inserted  PLS_INTEGER;

   CURSOR c_ml_config IS
       SELECT swms_ml_synonym, ml_system
   ...
   CURSOR c_hoi IS
      SELECT hoi_id, creation_timestamp, length, data
        FROM HOI
        --  where ml_system ='' || '''''''' || c1.ml_system || '''''''' || '')'';				
       ORDER BY hoi_id;
BEGIN
   DBMS_OUTPUT.PUT_LINE(''Executing p_split, miniloader using SQLSERVER and it is not OpCo 024...'');
   ...
   FOR c1 IN c_ml_config LOOP
      ...
      FOR r_hoi IN c_hoi LOOP

         IF (l_first_hoi_id IS NULL) THEN
            l_first_hoi_id := r_hoi.hoi_id;
         END IF;
         ...
         INSERT INTO hoi_hk1
                  (hoi_id,
                   ...
                   data )
           VALUES (r_hoi.hoi_id,
                   r_hoi.creation_timestamp,
                   r_hoi.length,
                   r_hoi.data);

         l_last_hoi_id  := r_hoi.hoi_id;
         l_number_of_records_inserted := l_number_of_records_inserted + 1;

      END LOOP;

      DBMS_OUTPUT.PUT_LINE(''Before COMMIT of insert to ML HOI table'');
      COMMIT;
      DBMS_OUTPUT.PUT_LINE(''After COMMIT of insert to ML HOI table'');

      DBMS_OUTPUT.PUT_LINE(''l_first_hoi_id: '' || TO_CHAR(l_first_hoi_id));
      DBMS_OUTPUT.PUT_LINE(''l_last_hoi_id: ''  || TO_CHAR(l_last_hoi_id));
      DBMS_OUTPUT.PUT_LINE(''l_number_of_records_inserted: '' || TO_CHAR(l_number_of_records_inserted));

      DBMS_OUTPUT.PUT_LINE(''======='');

      DBMS_OUTPUT.PUT_LINE(''Before DELETE from swms.hoi'');

      DELETE FROM hoi WHERE hoi_id BETWEEN l_first_hoi_id AND l_last_hoi_id; 

      DBMS_OUTPUT.PUT_LINE(''SQL%ROWCOUNT: '' || SQL%ROWCOUNT);
      DBMS_OUTPUT.PUT_LINE(''After DELETE from swms.hoi'');

      DBMS_OUTPUT.PUT_LINE(''Before COMMIT of DELETE'');
      COMMIT;
      DBMS_OUTPUT.PUT_LINE(''After COMMIT of DELETE'');
   END LOOP;
...
IS
   ...
   l_hoi_id  hoi.hoi_id%TYPE;
   l_creation_timestamp    hoi.creation_timestamp%TYPE;
   l_length  hoi.length%TYPE;
   l_data    hoi.data%TYPE;
BEGIN
   DBMS_OUTPUT.PUT_LINE(''Executing p_split, miniloader using SQLSERVER and it is OpCo 024...'');
   ...
   FOR c1 IN c_ml_config LOOP
      ...
      FOR r_hoi IN c_hoi LOOP
         ...
         ***/
         ...
         sqlStmt := '' INSERT INTO hoi_hk1
                  (hoi_id,
                   creation_timestamp,
                   length,
                   data )
            VALUES (:l_hoi_id, :l_creation_timestamp, :l_length, :l_data)'';

         EXECUTE IMMEDIATE sqlStmt using r_hoi.hoi_id, r_hoi.creation_timestamp, r_hoi.length, r_hoi.data;
      ...
      IF (l_first_hoi_id IS NOT NULL) THEN
         pl_log.ins_msg(''INFO'', ''p_split'',
                   ''After inserting into miniloader HOI  l_first_hoi_id: '' || TO_CHAR(l_first_hoi_id)
                || ''  l_last_hoi_id: ''  || TO_CHAR(l_last_hoi_id)
                || ''  l_number_of_records_inserted: '' || TO_CHAR(l_number_of_records_inserted),
                 sqlcode, null, ''INV'', ''p_split'');
      END IF;
      ...
      IF (l_first_hoi_id IS NOT NULL) THEN
         pl_log.ins_msg(''INFO'', ''p_split'',
                   ''before delete from swms.hoi  l_first_hoi_id: '' || TO_CHAR(l_first_hoi_id)
                || ''  l_last_hoi_id: ''  || TO_CHAR(l_last_hoi_id),
                 sqlcode, null, ''INV'', ''p_split'');
      END IF;
      ...
      DBMS_OUTPUT.PUT_LINE(''SQL%ROWCOUNT: '' || to_char(l_number_of_records_deleted));

      IF (l_first_hoi_id IS NOT NULL) THEN
         pl_log.ins_msg(''INFO'', ''p_split'',
                   ''after delete from swms.hoi  l_first_hoi_id: '' || TO_CHAR(l_first_hoi_id)
                || ''  l_last_hoi_id: ''               || TO_CHAR(l_last_hoi_id)
...
BEGIN
   ...
   DBMS_OUTPUT.PUT_LINE('pl_ml_split.sql: l_host_name: ' || l_host_name);
...
BEGIN
   ...
   IF (l_ml_database = 'SQLSERVER') THEN
      ...
      DBMS_OUTPUT.PUT_LINE(l_file_name || ': Create procedure p_split for SQLSERVER and it is not OpCo 024');
   ELSIF (l_ml_database = 'SQLSERVER_024') THEN
      l_sqlstmt_to_run := l_sqlstmt_sqlserver_024;
      DBMS_OUTPUT.PUT_LINE(l_file_name || ': Create procedure p_split for SQLSERVER and it is OpCo 024');
   ELSIF (l_ml_database  = 'ORACLE') THEN
      l_sqlstmt_to_run := l_sqlstmt_oracle;
      DBMS_OUTPUT.PUT_LINE(l_file_name || ': Create procedure p_split for ORACLE');
   ELSE
      l_sqlstmt_to_run := l_sqlstmt_oracle;   -- Default is oracle. as this will always compile.
      DBMS_OUTPUT.PUT_LINE(l_file_name || ': Default  Create procedure p_split for ORACLE');
   END IF;

   -------------DBMS_OUTPUT.PUT_LINE(l_sqlstmt_to_run);

   --
   -- If creating p_split fails output a message.  Do not raise an exception.
   -- Reasons it could fail is when the miniloader is using SQLSERVER and
   -- database link hk1.world does not exist or synonym hoi_hk1 does not exist
   -- or tnsmames.ora does not have the correct entries.
   -- On a test box these probably do not exist.
   -- Note that when creating p_split when the miniloader database is ORACLE should
   ...
   EXCEPTION
      WHEN OTHERS THEN
         DBMS_OUTPUT.PUT_LINE(l_file_name || ': Error creating p_split');

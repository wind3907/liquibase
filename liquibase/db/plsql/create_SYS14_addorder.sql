CREATE OR REPLACE  PROCEDURE "SWMS"."CREATE_SYS14_ADDORDER"  
      ...
      ----------------------------------------Correct the Query-------------------------------------------------------------
        CURSOR c_sos_batch_info IS
            SELECT vs.order_id, vs.route_no, vs.stop_no, vs.order_seq, vs.prod_id, TRUNC(vs.qty_alloc/vs.spc) case_qty, vs.batch_seq,
                   ...
                   vs.order_type, vs.orig_batch_no, vs.spc
--                   vs.carrier_id, vs.order_type, vs.no_of_floats
   ...
   BEGIN
        ...
        l_sys_msg_id := mx_sys_msg_id_seq.NEXTVAL;
            ...
            LOOP
                ...
                SELECT DECODE(rc.order_type, 'VRT', 'ABS','HIGH')
...
-- Find item in inventory
                ...
                BEGIN
                  SELECT rowid, plogi_loc, logi_loc, slot_type
                    INTO l_rowid, l_pick_loc, l_pallet_id, l_slot_type
                    FROM (SELECT i.rowid, i.plogi_loc, i.logi_loc, l.slot_type
                            FROM loc l, inv i
                           WHERE l.logi_loc = i.plogi_loc
                             AND l.slot_type IN ('MXF', 'MXC', 'MXS', 'MXI')
                             AND i.prod_id = rc.prod_id
                            AND (i.qoh + i.qty_planned - i.qty_alloc) > 0
                          ORDER BY i.exp_date, i.logi_loc)
                   WHERE ROWNUM = 1;

                  UPDATE inv
                     SET qty_alloc = qty_alloc + rc.spc, qoh = qoh - rc.spc
                   WHERE ROWID = l_rowid;
                ...
                EXCEPTION
                    WHEN NO_DATA_FOUND THEN
                        l_msg_text := 'Prog Code: ' || l_fname
                                || ' Unable to find sequence number of detail record (SYS14) record for prod_id '||rc.prod_id || ' and sys_msg_id '||l_sys_msg_id;
                        RAISE e_fail;
                END;

                --Populate case number and Label in table matrix_out_label
                BEGIN
                    FOR r_sc IN c_short_cases (rc.orig_batch_no, rc.order_seq)
                    LOOP

REM
REM File : v_truck_accessory_user.sql
REM
REM sccs_id = @(#) src/schema/views/v_truck_accessory_user.sql, swms, swms.9, 10.1.1 1/2/09 1.3 
REM
...
REM
CREATE OR REPLACE VIEW swms.v_truck_accessory_user AS
  SELECT truck truck_no,
	 ...
	 MAX(route_no) route_no,
         REPLACE(MAX(add_user), 'OPS$', '') add_user,
         MAX(add_date) add_date,
         REPLACE(MAX(upd_user), 'OPS$', '') upd_user,
         MAX(upd_date) upd_date,
  ...
  SELECT truck truck_no,
	 ...
	 manifest_no,
	 route_no,
         REPLACE(add_user, 'OPS$', '') add_user,
         add_date,
         REPLACE(upd_user, 'OPS$', '') upd_user,
         upd_date,
	 loader_count,
	 inbound_count
...
COMMENT ON TABLE v_truck_accessory_user IS 'VIEW sccs_id=@(#) src/schema/views/v_truck_accessory_user.sql, swms, swms.9, 10.1.1 1/2/09 1.3';

------------------------------------------------------------------------------
-- sccs_id=@(#) src/schema/views/v_merge_batch_time.sql, swms, swms.9, 10.1.1 9/7/06 1.4
--
-- View:
--    v_merge_batch_time
--
-- Description:
--    This view calculates the batch time for a merged non-discrete selection
...
--    results.  The same goes for the discrete selection views.
--
...
PROMPT Create view v_merge_batch_time

CREATE OR REPLACE VIEW swms.v_merge_batch_time
AS
SELECT b.parent_batch_no,
    (((SUM(DECODE(b.batch_no,
      ...
      (SUM(b.kvi_no_piece           *   j.tmu_no_piece))          +  
      ...
      (SUM(b.kvi_no_case            *   j.tmu_no_case))           + 
      (SUM(b.kvi_no_split           *   j.tmu_no_split))          + 
      (SUM(b.kvi_no_merge           *   j.tmu_no_merge))          +
      (SUM(b.kvi_no_aisle           *   j.tmu_no_aisle))          + 

------------------------------------------------------------------------------
-- sccs_id=@(#) src/schema/views/v_non_merge_batch_time.sql, swms, swms.9, 10.1.1 9/7/06 1.4
--
-- View:
--    v_non_merge_batch_time
--
-- Description:
--    This view calculates the batch time for a non-merged non-discrete
--    selection labor management batch by adding together the product of
--    the kvi's and tmu's.  There is a separate view for merged batches.
--    There could possibly be one view for both non-merged and merged
--    batches but I am not sure it would always return the correct results.
--    The same goes for the discrete selection views.
--
...
--    -------- -------- ---------------------------------------------------
--    02/06/04 prpbcb   Oracle 7 rs239a DN None.  Does not exist on oracle 7.
...
--    06/28/04 prpbcb   Add clam bed data capture.
--    08/30/04 prpbcb   Added shorts.
------------------------------------------------------------------------------

PROMPT Create view v_non_merge_batch_time

CREATE OR REPLACE VIEW swms.v_non_merge_batch_time
AS
SELECT b.batch_no, 
       b.jbcd_job_code, 
       b.status,
    ...
    (((1                        *   j.tmu_doc_time)           +
      ...
      (b.kvi_no_case            *   j.tmu_no_case)            + 
      (b.kvi_no_split           *   j.tmu_no_split)           + 
      (b.kvi_no_merge           *   j.tmu_no_merge)           +
      (b.kvi_no_aisle           *   j.tmu_no_aisle)           + 
 ...
 (b.kvi_no_clam_bed_data_capture    *   j.tmu_no_clam_bed_data_capture)
       ...
       b.kvi_distance batch_time_in_minutes,
       b.kvi_distance,
       b.ds_case_time,
       b.ds_split_time,
       b.goal_time,

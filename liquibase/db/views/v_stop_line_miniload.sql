...
--
------------------------------------------------------------------------------
CREATE OR REPLACE VIEW swms.v_stop_line_miniload
(
   route_no,
   order_id,
   order_line_id,
...
SELECT fl_item.route_no,
  ...
  FROM (  -- start inline view
        SELECT s.route_no,
               ...
               END)   auto_ship_flag,
               --
               --  08/218/2013 BRian Bent  Old stmt.
               -- DECODE(p.miniload_storage_ind, 'B', 'Y',
               --                                'S', 'Y',
               --                                 p.auto_ship_flag) auto_ship_flag,
               --
               r.method_id,
               r.truck_no,
               r.f_door,
          ...
          FROM route r,
               ...
               pm p,
               --
               -- 08/26/2013  Brian Bent  Inline view to select the syspar that controls
               ...
               -- split order.
               --
               (SELECT NVL(MIN(config_flag_val), 'N') case_up_split_order
                  FROM sys_config 
                 WHERE config_flag_name = 'CASE_UP_MINILOAD_SPLIT_ORDER') syspar
               --
         WHERE ((p.miniload_storage_ind = 'S' AND s.uom = 1) OR p.miniload_storage_ind = 'B')
...
CREATE OR REPLACE PUBLIC SYNONYM v_stop_line_miniload
FOR swms.v_stop_line_miniload
/
